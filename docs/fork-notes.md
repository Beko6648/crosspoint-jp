...
Xteink X3用に、本家`v0.4.0`(コミット`2590620`)をベースに統合した開発版（`main`ブランチ）です。フォーク独自機能（インライン画像・ページ送り中インジケータ・章一覧のTOC除外・縦書き段落配置の固定など）を維持しています。なお、一部の上流機能は意図的に変更したり取り込んでいません（下記「意図的に取り込んでいない上流機能」参照）。GitHub Releasesで配布中の最新版は`yomuka-v0.5.0-fork1.6`(上流ベース`v0.5.0`、FreeInk表示SDK移行済み)です。
...
- **FreeInk表示SDKへ移行**(2026-08-23):表示基盤を `open-x4-sdk` から `freeink-sdk` に差し替え(`.gitmodules`・`platformio.ini`・`HalDisplay`/`HalGPIO`/`main.cpp`)。従来の縦書き・ルビ・SDカードフォント・`SLEEP_REFRESH`・画像ゴースト対策を維持。`freeink-sdk` は入れ子submodule(`libs/assets/Icons/lucide`)を含むため、`git submodule update --init --recursive` が必要。
  - **X3表示コントローラー判別**:起動時に搭載パネルの表示コントローラー(UC8253 / UC8279)を判別し、診断画面の`Device:`と診断レポートに表示。
  - **スリープ時の電源レール制御**:スリープ時の電源レールを制御して消費電力を削減。X3でRTC有効時はRTC共有電源を保持し復帰後の時刻を維持。
- **EPUB安定性・互換性**: ファイル一覧の描画競合修正(`RenderLock`導入)・明示ページ移動位置の巻き戻り防止(`clearDeferredReposition`)・`&apos;`HTMLエンティティ対応・ルビグループの分割欠落修正・章末の不要マークアップ読み飛ばし・長い脚注リンク対応・独自名前空間EPUBメタデータ対応。
