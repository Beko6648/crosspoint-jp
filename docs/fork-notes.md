...
Xteink X3用に、本家`v0.4.0`(コミット`2590620`)をベースに統合した開発版（`main`ブランチ・`v0.4.0-fork1.0`）です。フォーク独自機能（インライン画像・ページ送り中インジケータ・章一覧のTOC除外・縦書き段落配置の固定など）を維持しています。なお、一部の上流機能は意図的に変更したり取り込んでいません（下記「意図的に取り込んでいない上流機能」参照）。GitHub Releasesで配布中の最新版は`yomuka-v0.4.0-fork1.1`(上流ベース`v0.4.0`)です。
...
- **ファイル一覧で .bin（ファームウェア）を整理可能**(2026-08-19):本のファイル一覧(SDカード画面)に`.bin`も表示し、ロングプレスによる**削除(右)・アーカイブ(`/Archived/`へ移動・左)**ができるようにした(※左ボタンは後に下記「移動」機能へ置き換え)。従来は`.bin`が本一覧に表示されず整理できなかった(FWは「SDカードファームウェア更新」の選択画面でのみ表示され、そこに削除機能は無い)。`.bin`は本ではないため、ショートプレスで開こうとしない・「既読にする」も提供しない(ディレクトリと同様にbtn2を空に)。アイコンは通常のファイルアイコン、既読ステータス・キャッシュアイコンは`.epub`限定のため影響なし。実機確認済み(2026-08-19)。
- **ファイル一覧の「移動」機能（アーカイブを置き換え）**(2026-08-19):本のファイル一覧のロングプレス操作で、左ボタンを従来の「アーカイブ」から「**移動**」に変更し、ファイル/フォルダを**任意のフォルダへ**移動できるようにした(commit `cf20533`)。移動先は、一覧先頭に常時表示される「このフォルダへ移動」を選んでフォルダを辿る方式(`Mode::PickDirectory`)で、その状態の確定ボタンは**「決定」**(`STR_OK_BUTTON`・横幅で切れない短い文言)。移動先に**同名**のファイル/フォルダがある場合は**拒否**(メッセージ表示・移動しない)、**自身やその配下への移動も拒否**(循環防止)。`.epub`は移動先でキャッシュを無効化して再生成。`.bin`も移動対象。※実装当初、移動先選択の仮想エントリ(`"\x01"`・拡張子なし)を一覧に出すと、`getFileExtension` が `substr(npos)` → `std::out_of_range` → 本体が abort する不具合があり、`npos` ガードを追加して修正。実機確認済み(2026-08-19)。
