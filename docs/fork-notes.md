...
- **ファイル一覧で .bin（ファームウェア）を整理可能**(2026-08-19):本のファイル一覧(SDカード画面)に`.bin`も表示し、ロングプレスによる**削除(右)・アーカイブ(`/Archived/`へ移動・左)**ができるようにした。従来は`.bin`が本一覧に表示されず整理できなかった(FWは「SDカードファームウェア更新」の選択画面でのみ表示され、そこに削除機能は無い)。`.bin`は本ではないため、ショートプレスで開こうとしない・「既読にする」も提供しない(ディレクトリと同様にbtn2を空に)。アイコンは通常のファイルアイコン、既読ステータス・キャッシュアイコンは`.epub`限定のため影響なし。実機確認済み(2026-08-19)。
