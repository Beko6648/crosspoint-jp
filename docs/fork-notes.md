...
Xteink X3用に、本家`v0.4.0`(コミット`2590620`)をベースに統合した開発版（`main`ブランチ・`v0.4.0-fork1.0`）です。フォーク独自機能（インライン画像・ページ送り中インジケータ・章一覧のTOC除外・縦書き段落配置の固定など）を維持しています。なお、一部の上流機能は意図的に変更したり取り込んでいません（下記「意図的に取り込んでいない上流機能」参照）。GitHub Releasesで配布中の最新版は`yomuka-v0.4.0-fork1.4`(上流ベース`v0.4.0`)です。
