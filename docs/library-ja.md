...
**本の履歴 → 本の履歴** で対象の本を選び、画面に表示される **選択** を長押しすると、その本を履歴から削除する確認画面が開きます。一覧末尾の **読書データをすべて消去** では、すべての読書履歴と読書時間を削除できます。

この操作は履歴と読書時間を消去します。SDカード上の書籍ファイルを削除する操作ではありません。消去した履歴は元に戻せないため、確認画面の対象を確認してから実行してください。
