...
struct TestWire {
  ...
  void write(uint8_t reg) { selected=reg;reads.push_back(reg); }
  int endTransmission(bool stop) { assert(!stop);return selected==failReg?1:0; }
  void requestFrom(uint8_t addr,uint8_t n) { assert(addr==0x55&&n==2);count=selected==shortReg?1:2;offset=0; }
