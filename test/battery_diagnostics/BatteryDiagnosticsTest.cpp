...
int main() {
...
#endif
  logs.clear();Wire.reads.clear();testMillis+=1500;Wire.values[0x2c]=7;
  ...
  assert(logs.back().find("old_soc=84 soc=7 voltage_mV=3700 current_mA=-120 remaining_mAh=210 full_mAh=300 valid=0xF")!=std::string::npos);
  // Failed SOC writes / short reads preserve the last display and last successful raw SOC.
  Wire.failReg=0x2c;testMillis+=1500;Wire.values[0x2c]=55;
  assert(pm.getBatteryPercentage()==7);
  Wire.failReg=-1;Wire.shortReg=0x2c;testMillis+=1500;
  assert(pm.getBatteryPercentage()==7);
  Wire.shortReg=-1;testMillis+=1500;
  ...
  for(int reg:{0x08,0x0c,0x10,0x12}) {
    Wire.failReg=reg;testMillis+=1500;Wire.values[0x2c]=Wire.values[0x2c]==55?80:55;
  ...
  testMillis+=1500;Wire.values[0x2c]=0;assert(pm.getBatteryPercentage()==0);
  logs.clear();testMillis+=1500;Wire.values[0x2c]=14;assert(pm.getBatteryPercentage()==14);
#if LOG_LEVEL < 2
  assert(logs.empty());
#endif
  testMillis+=1500;Wire.values[0x2c]=29;assert(pm.getBatteryPercentage()==29);
  assert(logs.back().find("SOC_JUMP")!=std::string::npos);
  testMillis+=1500;Wire.values[0x2c]=200;assert(pm.getBatteryPercentage()==100);
