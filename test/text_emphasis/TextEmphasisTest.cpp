...
int main(int argc,char** argv) {
 ...
 assert(textEmphasis::parse("open",e)&&textEmphasis::codepoint(e,true)==0xfe46&&textEmphasis::codepoint(e,false)==0x25e6);
 assert(textEmphasis::parse("filled sesame black",e,true)&&e==E::FilledSesame);
 for(auto s:{"", "invalid", "inherit", "none dot", "open filled", "dot circle", "'x'"}) assert(!textEmphasis::parse(s,e));
 CssStyle parent,child; parent.emphasis=E::FilledSesame;parent.emphasisDefined=true;
 child.emphasis=E::None;child.emphasisDefined=true;parent.applyOver(child);assert(parent.emphasis==E::None&&parent.anySet());
 parent.reset();assert(!parent.emphasisDefined);
 for(auto name:{"text-emphasis","text-emphasis-style","-epub-text-emphasis","-epub-text-emphasis-style","-webkit-text-emphasis","-webkit-text-emphasis-style"}) {
 ...
 FsFile css;Storage.openFileForWrite("TEST","/source",css);
 const std::string rules=".mark{text-emphasis:filled sesame;color:black}.off{text-emphasis:none}.other{width:40%;font-weight:bold}";
 css.write(reinterpret_cast<const uint8_t*>(rules.data()),rules.size());
 CssParser parser("/cache");parser.setCacheSourceFingerprint(123);assert(parser.loadFromStream(css));assert(parser.saveToCache());
 CssParser restored("/cache");restored.setCacheSourceFingerprint(123);assert(restored.loadFromCache());
 assert(restored.resolveStyle("span","mark").emphasis==E::FilledSesame);
 assert(restored.resolveStyle("span","off").emphasisDefined&&restored.resolveStyle("span","off").emphasis==E::None);
 assert(restored.resolveStyle("span","other").imageWidth.value==40);
 std::string cachePath;for(const auto& item:Storage.files) if(item.first.find("/cache/")==0)cachePath=item.first;
 ...
 for(size_t length=0;length<good.size();++length) {
   ...
   CssParser truncated("/cache");truncated.setCacheSourceFingerprint(123);assert(!truncated.loadFromCache());
 }
 Storage.files[cachePath]=std::make_shared<std::vector<uint8_t>>(good);(*Storage.files[cachePath])[0]=7;
 ...
 GfxRenderer fallback; fallback.fonts[1].glyphs[0x2022]={};
 ...
 GfxRenderer exact;exact.fonts[1].glyphs[0xfe45]={};
 ...
 GfxRenderer rubyOff;withRuby.renderEmphasis(rubyOff,1,20,30);assert(ink(rubyOff)>0);
 GfxRenderer longRun;
 block(std::string(52,'A'),E::FilledDot).renderEmphasis(longRun,1,20,30);
 assert(ink(longRun)==ink(missing)*52);
 GfxRenderer punct;block(" ,.!",E::FilledDot).renderEmphasis(punct,1,20,30);assert(ink(punct)==0);
 GfxRenderer scan;scan.cache.scanning=true;block("A",E::FilledDot).renderEmphasis(scan,1,20,30);assert(ink(scan)==0);
 auto v=block("A",E::FilledDot,true);
 assert(v.annotationRightOverflow(missing,1,25)>0&&v.annotationTopInset(missing,1)>0);
 GfxRenderer vertical;v.renderEmphasis(vertical,1,20,30);assert(ink(vertical)>0);
 GfxRenderer verticalRun;block("ABC",E::FilledDot,true).renderEmphasis(verticalRun,1,20,30);
 ...
 if(argc>1) {
   ...
   std::ofstream out(argv[1],std::ios::binary);out<<"P5\n"<<GfxRenderer::W<<" "<<GfxRenderer::H<<"\n255\n";
   out.write(reinterpret_cast<const char*>(sheet.pixels.data()),sheet.pixels.size());
 }
 std::cout<<"PASS: CSS/cache roundtrip and truncation, inheritance, font fallback, geometric fallback, 52 characters, punctuation, scan and vertical placement\n";
}
