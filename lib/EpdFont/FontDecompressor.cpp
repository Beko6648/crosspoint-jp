...
bool FontDecompressor::ensureCapacity(uint8_t*& buffer, uint32_t& capacity, const uint32_t needed) {
  if (capacity >= needed) return true;

  // Every grow is followed by a full rewrite, so discarding the old buffer
  // maximizes the available contiguous allocation on the constrained heap.
  free(buffer);
...
const uint8_t* FontDecompressor::getBitmap(const EpdFontData* fontData, const EpdGlyph* glyph, uint32_t glyphIndex) {
  ...
  if (!(hotGroup != nullptr && hotGroupFont == fontData && hotGroupIndex == groupIndex)) {
    ...
    // ensureCapacity may free the old group, so do not retain its identity.
    hotGroupFont = nullptr;
    hotGroupIndex = UINT16_MAX;
  ...
  if (!ensureCapacity(hotGlyphBuf, hotGlyphBufCapacity, glyph->dataLength)) {
    LOG_ERR("FDC", "Failed to allocate %u bytes for glyph scratch", static_cast<unsigned>(glyph->dataLength));
    stats.getBitmapTimeUs += micros() - tStart;
    return nullptr;
  }
