...
XtcError XtcParser::readHeader() {
  ...
  // A compact 48-byte header has no chapterOffset or padding.  The final
  // eight bytes read into XtcHeader are already the first page-table entry,
  // so never interpret them as chapter metadata.
  if (m_header.pageTableOffset >= XTC_MIN_HEADER_SIZE && m_header.pageTableOffset < sizeof(XtcHeader)) {
...
uint64_t XtcParser::metadataBase() {
  ...
  if (base < XTC_MIN_HEADER_SIZE || base + XTC_METADATA_SIZE > fileSize) return 0;
  // Metadata must not overlap the page index.  Compact headers commonly have
  // no metadata at all, despite an arbitrary byte sequence after offset 48.
  if (m_header.pageTableOffset > base && base + XTC_METADATA_SIZE > m_header.pageTableOffset) return 0;
  return base;
}
...
XtcError XtcParser::readTitle() {
  ...
  char titleBuf[XTC_TITLE_SIZE] = {0};
...
XtcError XtcParser::readAuthor() {
  ...
  if (!m_file.seek(static_cast<size_t>(base + XTC_TITLE_SIZE))) {
    return XtcError::READ_ERROR;
  }

  char authorBuf[XTC_AUTHOR_SIZE] = {0};
...
XtcError XtcParser::readChapters() {
  ...
  const uint64_t fileSize = m_file.size();
  if (chapterOffset < XTC_MIN_HEADER_SIZE || chapterOffset >= fileSize || chapterOffset + 96 > fileSize) {
    return XtcError::OK;
  }
