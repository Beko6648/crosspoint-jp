...
void BookReaderSettingsActivity::selectCurrent() {
  ...
  if (success && item == Item::SaveAll) {
    ...
    switch (item) {
      ...
      case Item::TateChuYokoDigits: toggleDirection(direction, currentDirection, kTateChuYokoFields); break;
...
bool BookReaderSettingsActivity::isOverridden(const Item item) const {
  ...
  switch (item) {
    ...
    case Item::TateChuYokoDigits: return hasDirection(direction, kTateChuYokoFields);
...
int BookReaderSettingsActivity::itemCount() const {
  return static_cast<int>(Item::Count) - (verticalMode ? 0 : 1);
}
