...
void RecentBooksActivity::render(RenderLock&&) {
  ...
  if (screen == Screen::Menu) {
    ...
    GUI.drawList(
        renderer, Rect{0, contentTop, pageWidth, contentHeight}, HISTORY_MENU_ITEM_COUNT, menuIndex,
    ...
    if (isX3) {
      ...
      if (meterPage == MeterPage::Overview || !summary.hasCalendarTime) {
        const std::string primaryLabel =
            summary.hasCalendarTime ? tr(STR_READING_METER_WEEK) : tr(STR_READING_METER_TOTAL);
      ...
      if (summary.hasCalendarTime)
        renderer.drawCenteredText(UI_10_FONT_ID, contentTop + contentHeight - 25, total.c_str());
    }
  ...
  const bool x3MeterPaging =
      screen == Screen::Meter && gpio.deviceIsX3() && READING_HISTORY.getSummary().hasCalendarTime;
