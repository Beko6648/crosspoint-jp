...
bool AozoraIndexManager::loadAndPurge() {
  ...
  // Older firmware writes JSON through .tmp/.bak. Recover it before deciding
  // whether it needs migration, so an interrupted pre-binary save is never
  // mistaken for an empty history.
  if (!recoverLegacyJsonIndexFiles_()) {
    return false;
  }
...
bool AozoraIndexManager::recoverLegacyJsonIndexFiles_() {
  constexpr const char* legacyTmpPath = "/Aozora/.aozora_index.json.tmp";
  constexpr const char* legacyBackupPath = "/Aozora/.aozora_index.json.bak";

  if (Storage.exists(legacyBackupPath)) {
    if (Storage.exists(INDEX_PATH)) {
      LOG_DBG("AOZORA", "Removing stale legacy JSON backup");
      Storage.remove(legacyBackupPath);
    } else if (!Storage.rename(legacyBackupPath, INDEX_PATH)) {
      LOG_ERR("AOZORA", "Failed to restore legacy JSON backup");
      return false;
    }
  }
  if (Storage.exists(legacyTmpPath)) {
    LOG_DBG("AOZORA", "Removing stale legacy JSON temporary file");
    Storage.remove(legacyTmpPath);
  }
  return true;
}

bool AozoraIndexManager::recoverEntryFile_(const AozoraBookEntry& entry) {
  char fullPath[160];
  char backupPath[176];
  ...
  if (Storage.exists(backupPath)) {
    ...
    LOG_ERR("AOZORA", "Failed to restore update backup: %s", entry.filename);
  }
  return false;
}
