...
> **Beko6648フォークの開発状況メモ**
>
> Xteink X3用に、本家`v0.4.0`(コミット`2590620`)をベースに統合した開発版で、フォーク独自機能（インライン画像・ページ送り中インジケータ・章一覧のTOC除外・縦書き段落配置の固定など）を維持しています。GitHub Releasesで配布中の最新版は`yomuka-v0.5.0-fork1.6`(上流ベース`v0.5.0`、FreeInk表示SDK移行済み)です。**変更の詳細・技術的経緯は [フォーク開発状況メモ(詳細)](docs/fork-notes.md) を参照してください。**
>
> **修正済み（要約）**
>
> **上流 v0.5.0 取り込み（2026-08-23・実機確認済み）**
> - **FreeInk表示SDKへ移行**(open-x4-sdk→freeink-sdk差し替え)＋X3の表示コントローラー判別(UC8253/UC8279)＋スリープ時電源レール制御。従来の縦書き・ルビ・SDカードフォント・SLEEP_REFRESHを維持
> - **EPUB安定性**: 描画競合修正・ページ位置の巻き戻り防止・`&apos;`エンティティ対応・ルビグループ分割修正・長い脚注リンク対応・独自名前空間メタデータ対応
> - **メモリ改善**: 使用中CSSルールだけ読込、使用中フォントだけ事前読込、画像デコード安全化、EPUB最適化での埋込フォント/コメント除去
> - **透過スリープ画像**: `/sleep-overlay.bmp` または `/.sleep-overlay/` フォルダの複数BMPをスリープごとにランダム表示
>
> **ページ描画・画像**
...
> - **drawArc/fillArc 描画の高速化**(上流#1540移植、2026-08-20):円弧描画を走査ベースに変更。実機確認済み(2026-08-20)
...
> - **SDフォントのkern行キャッシュと縦字形先読み**(上流 v0.4.1 移植、2026-08-20):メモリ断片化時のkern確保失敗を抑制し、転送直後の読点・括弧の向き崩れを防止。実機確認済み
>
> **キャッシュ**
...
>
> **シリアル/USB**
> - **バッテリー満充電時のUSB Serial不活性を修正**(上流 zrn-ns PR#69 移植、2026-08-20):X3 でバッテリー100%だと充電電流0mAでUSB接続判定が外れ、`Serial.begin()` が呼ばれずフラッシュ・シリアル監視不能になる問題を、Serial 初期化の無条件化+250ms delay+`setTxTimeoutMs(1)` で修正
>
