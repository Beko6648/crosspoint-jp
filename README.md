...
## Beko6648フォークでの開発状況メモ

Xteink X3用に、`v0.3.7`(コミット`124a5da`)をベースとして以下の修正を加えています。

### 修正済み

- **XTCHスリープ表紙のグレースケール化**:スリープ時に表示する表紙を、従来の2値ではなく4階調(白・濃灰・薄灰・黒)で表示するよう修正
- **文字重なり(幅0)の修正**:`SECTION_FONT_CODEPOINT_LIMIT=1024`によりadvance tableから漏れた漢字(例:縛・鎖・堅)が幅0で重なる問題を修正。漏れた漢字には**標準全角幅**を返す方式(`getAdvanceOrLoad` / `getFullWidthAdvance`)。SD読み・ヒープ確保を行わないため、ヒープ断片化を起こさない
- **キャッシュ生成終了時のフォントキャッシュ解放**:`GenerateAllCacheActivity::onExit`で`clearCache()`と`freeKernLigatureData()`を呼び、キャッシュ生成後に残るフォントキャッシュ(~130KB)を解放

### 未解決(既知の問題)

- **XTCHの「メモリエラー」**:キャッシュ生成プロセスがヒープを**断片化**させることが原因と判明しています。
  - 再現手順:XTCHを開く → 他EPUBのキャッシュ生成 → 最初のXTCHを再度開く →「メモリエラー」
  - 診断結果:空きヒープ`free=155704`なのに最大連続ブロック`maxblk=55284`(=断片化)。XTCHのページバッファ(約96KB)の連続確保が失敗
  - 幅0回避とは**無関係**(検証済み)。EPUB読書には影響しないため、一旦保留
  - シリアルログが取れないため、断片化の発生箇所(レイアウト計算/画像生成)の特定は未完了

### ブランチ

- `fix/xtch-grayscale-cover-v037`:上記の修正をすべて含む
