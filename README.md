...
> - **画像ページの描画速度を改善**(14秒→4秒・実機確認済み):本文中に多数の小さなインライン画像(JPEG)が埋め込まれたページの描画が極端に遅い問題を2点修正。①`ImageBlock::render`が**JPEGで有効なBMPキャッシュを読む場合でも毎回フォントキャッシュを全破棄**していたのを、`hasValidBmpCache`が真のときは破棄をスキップ(画像のたびに直後のテキストの全グリフをSDから再読み込みするストームを解消)。②`GfxRenderer::drawBitmap`の縮小処理を**出力側走査＋2D領域サンプリング**(縦横をstrideで間引きつつ最暗値を採用)に最適化し、502×502→22×22のような極端な縮小でも**細い縦線・横線を保ったまま約1/60に高速化**。
>
> **未解決(既知の問題)**
