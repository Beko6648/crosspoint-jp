...
> **Beko6648フォークの開発状況メモ**
>
> **ベースの上流バージョン**: 上流 **`v0.6.5.2`**。フォーク独自機能（インライン画像・ページ送り中インジケータ・章一覧のTOC除外・縦書き段落配置の固定・三点リーダー改行修正など）を維持しています。**変更の詳細・技術的経緯は [フォーク開発状況メモ(詳細)](docs/fork-notes.md) を参照してください。**
>
...
- 縦書きで補助漢字（例：`𠮟`）を一文字として直立表示します。SDフォントで表示できる補助漢字が、後続のかなと一緒に横倒しになったり空白になったりする問題を修正しました。
- SDフォントに字形がない文字を `?` で代替するとき、レイアウトも同じ幅を使うようにしました。文字の重なりや列からのはみ出しを防ぎます。
- `ゎ ヮ ゕ ゖ ヵ ヶ` とアイヌ語用小書きカタカナを、縦書きの行頭禁則の対象に追加しました。
- EPUBの異常なZIP圧縮データを開いたとき、読み込み失敗を安全に処理します。
- XTCの読書位置を一時ファイル経由で保存し、電源断や書き込み中断で進捗ファイルが壊れにくくなりました。

### キャッシュの再生成

縦書きレイアウトの変更により、v0.6.4でEPUBを初めて開く際は、対象の読書キャッシュが自動で再生成されます。読書位置と設定は保持されます。
