...
> **Beko6648フォークの開発状況メモ**
>
> **ベースの上流バージョン**: 上流 **`v0.6.0`**（SDカードフォント配布方式の見直し）。GitHub Releasesで配布中の最新版は`yomuka-v0.6.0-fork1.7`。フォーク独自機能（インライン画像・ページ送り中インジケータ・章一覧のTOC除外・縦書き段落配置の固定など）を維持しています。**変更の詳細・技術的経緯は [フォーク開発状況メモ(詳細)](docs/fork-notes.md) を参照してください。**
>
...
日本語フォントは容量が大きいため、[SD Card Fonts Release](https://github.com/ponto1216-ai/crosspoint-jp/releases/tag/sd-fonts)から使用したいフォントの`Yomuka-Font-<フォント名>.zip`をPCでダウンロードし、SDカード直下へ展開する方法を推奨します。

ZIPには`.fonts`フォルダが含まれているため、フォルダを手作業で作る必要はありません。
...
手動で配置する場合は、フォントごとに専用のフォルダを作成し、その中へ各サイズの`.cpfont`ファイルを配置してください。

端末の「設定 > システム > フォントをダウンロード」からも導入できます。ただし、日本語フォントは完了まで時間がかかる場合があります。
...
新しく配置する場合は、端末の自動ダウンロード先と同じ`/.fonts/`の使用をおすすめします。
